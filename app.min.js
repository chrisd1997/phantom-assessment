...
function m(a,b){if(b)a:{var c=k;a=a.split(".");for(var d=0;d<a.length-1;d++){var e=a[d];if(!(e in c))break a;c=c[e]}a=a[a.length-1];d=c[a];b=b(d);b!=d&&null!=b&&f(c,a,{configurable:!0,writable:!0,value:b})}}m("Object.is",function(a){return a?a:function(b,c){return b===c?0!==b||1/b===1/c:b!==b&&c!==c}});
m("Array.prototype.includes",function(a){return a?a:function(b,c){var d=this;d instanceof String&&(d=String(d));var e=d.length;c=c||0;for(0>c&&(c=Math.max(c+e,0));c<e;c++){var g=d[c];if(g===b||Object.is(g,b))return!0}return!1}});
...
function r(a){var b=a.getAttribute("data-edit-id"),c=null;""!==b&&null!==b&&(c=p.filter(function(d,e){return e===parseInt(b)})[0]);a.value.length?/^(https?:\/\/)?((([a-z\d]([a-z\d-]*[a-z\d])*)\.)+[a-z]{2,}|((\d{1,3}\.){3}\d{1,3}))(\:\d+)?(\/[-a-z\d%_.~+]*)*(\?[;&a-z\d%_.~+=-]*)?(\#[-a-z\d_]*)?$/i.test(a.value)?!c&&p.includes(a.value)||c&&a.value!==c&&p.includes(a.value)?t(a,"URL is already bookmarked"):(t(a,null),v(a)):t(a,"Please provide a valid URL"):t(a,"This field cannot be empty")}
...
function w(a){a.preventDefault();a=a.target.querySelector('input[type="url"]');var b=a.getAttribute("data-edit-id"),c=p;""!==b&&null!==b?c[parseInt(a.getAttribute("data-edit-id"))]=a.value:c.push(a.value);x(c);a=a.value;b=document.getElementsByClassName("page-complete")[0];document.getElementsByClassName("page-home")[0].classList.remove("active");b.querySelector("#bookmark-submission").innerText='Your bookmark "'+a+'" has been added successfully!';b.classList.add("active")}
function y(a){var b=p.filter(function(d,e){return e===a})[0];if(b){var c=document.getElementById("url");c.value=b;c.setAttribute("data-edit-id",a.toString());c.focus();document.getElementsByClassName("cancel-edit")[0].classList.add("show")}}function z(a){p.length&&!0===confirm("Are you sure that you want to remove this bookmark?")&&(x(p.filter(function(b,c){return c!==a})),A(),B())}
function A(){var a=document.getElementsByTagName("table")[0];a.querySelector("tbody").innerHTML="";if(p.length){a=new URLSearchParams(window.location.search);a=parseInt(a.get("page"))||1;var b=20*a;p.slice(20===b?0:b-20,b).forEach(function(e,g){var n=document.createElement("tr"),u=document.createElement("td"),l=document.createElement("a");l.href=e.toString();l.target="_blank";l.appendChild(document.createTextNode(e));u.appendChild(l);e=document.createElement("td");e.innerHTML='<button type="button" class="edit-button" data-id="'+
    g+'"><i class="fal fa-pencil-alt"></i></button><button type="button" class="delete-button" data-id="'+g+'"><i class="fal fa-trash-alt"></i></button>';n.appendChild(u);n.appendChild(e);document.getElementsByTagName("table")[0].querySelector("tbody").appendChild(n)});document.getElementById("counter").innerText="Showing "+(b-20)+" - "+b;b=document.getElementById("prevPage");var c=document.getElementById("nextPage");1===a&&(b.classList.add("disabled"),b.href="#");1<a&&1<q&&(b.classList.remove("disabled"),
...
(function(){var a=document.getElementById("url");document.getElementById("add-form").addEventListener("submit",w);a.addEventListener("blur",function(b){return r(b.target)});a.addEventListener("keyup",function(b){return r(b.target)});B();A();document.addEventListener("click",function(b){b.target&&b.target.classList.contains("edit-button")?y(parseInt(b.target.dataset.id)):b.target&&b.target.classList.contains("delete-button")?z(parseInt(b.target.dataset.id)):b.target&&b.target.classList.contains("cancel-edit")&&
    (a.removeAttribute("data-edit-id"),document.getElementsByClassName("cancel-edit")[0].classList.remove("show"),a.value="",document.querySelector('button[type="submit"]').disabled=!0)})})();